import { guardar_carrito, obtener_carrito, vaciar_carrito } from "./api_carrito.js";
import { actualizar_contador_carrito, mostrarMensaje } from "./ui.js";

export  const agregar_a_carrito = (producto) => {
    let carrito = obtener_carrito();
    const producto_a_agregar = carrito.find(p => p.producto.productoId === producto.productoId);

    if(producto_a_agregar === undefined){
        carrito.push({producto : producto, cantidad: 1});
    }
    else{
        carrito = carrito.map(p => {
            if(p.producto.productoId === producto.productoId){
                p.cantidad += 1;
            }
            return p;
        });
